...
void sendPDU(int sockfd,PDU *pdu)
{
	...
	while(iLeft)
	{
		...
		{
			ret = write(sockfd,(char *)pdu+iSended,PER_MAX_IO_BYTES);
		}
		else
		{
			ret = write(sockfd,(char *)pdu+iSended,iLeft);
		}
		if(-1==ret)
		{
...
PDU *recvPDU(int sockfd)
{
	...
	while(iLeft)
	{
		...
		{
			ret = read(sockfd,(char *)pdu+iRecved,PER_MAX_IO_BYTES);
		}
		else
		{
			ret = read(sockfd,(char *)pdu+iRecved,iLeft);
		}
		if(-1 == ret||0 == ret)
		{
			break;
