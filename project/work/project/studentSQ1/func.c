...
{
	int index=0,i=0;
	if(is_list_empty_operation(l)==-1)
		return -1;
	index=is_list_empty_operation(l);
	if(index==-1)
		return -1;
	if(p_stu!=NULL)
		*p_stu=l->data[index];
...
//输入学生信息
int input_student_info(line_list_t *l,student_t *p_stu)
{
	int i=0;
	if(p_stu==NULL)
		return -1;
	...
	for(i=0;i<l->count;i++)
	{
		if(l->data[i].id==p_stu->id)
		{
			printf("您输入的ID已存在，请重新输入！！\n");
			return -1;
		}
	}
	printf("input name:");
...
//判空以后的处理
//成功，返回index;失败，返回-1
int is_list_empty_operation(line_list_t *l)
{
	if(is_list_empty(l))
		return -1;
	int index=0,id=0;
	printf("input id you want to operation:");
	scanf("%d",&id);
